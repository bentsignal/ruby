...
export function useLocationSearch({
  ...
  // eslint-disable-next-line no-restricted-syntax -- The parent opens this controlled dialog directly, so the search session must follow that external open state.
  useEffect(() => {
...
function loadPredictions({
  ...
  if (!isOpen || !sessionToken || input.length < 3) {
